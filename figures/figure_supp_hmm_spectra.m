...
for k=1:K
  if use_WB_nnmf
    scatter((squeeze((nanmean((psd_wb(:,k,:)),3)))), (squeeze(nanmean(coh_wb(:,k,offdiagselect),3))),15, 'MarkerFaceColor', color_scheme{k}, 'MarkerEdgeColor', 'None', 'MarkerFaceAlpha', 0.7);
  else
    scatter((squeeze((nanmean(nanmean((psd(:,k,:,:)),4),3)))), (squeeze(nanmean(nanmean((coh(:,k,:,offdiagselect)),4),3))),15, 'MarkerFaceColor', color_scheme{k}, 'MarkerEdgeColor', 'None', 'MarkerFaceAlpha', 0.7);
  end
...
for k=1:K
  if use_WB_nnmf
    ...
    scatter((squeeze((nanmean(nanmean((psd(:,k,:,:)),1),3)))), (squeeze(nanmean(nanmean(nanmean((coh(:,k,:,:,:)),5),3),1))),15, 'MarkerFaceColor', color_scheme{k}, 'MarkerEdgeColor', 'None', 'MarkerFaceAlpha', 0.7);
  end
  l{k} = sprintf('State %d', k);
end
% axis off
