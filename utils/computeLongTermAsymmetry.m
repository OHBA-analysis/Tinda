function [FO,pvals,tintervalsout, stat, FO_residual,pvals_residual, stat_res] = computeLongTermAsymmetry(vpath,T,K,intervalbins, dotaskglm, trialdef, dostat, ntiles, intervalbin_mode, replay_state)
...
FO_all = [];
if dotaskglm==1
  FO_residual = zeros(K+replay_state,K+replay_state,2,nSj,length(intervalbins)-1);
else 
  FO_residual=[]; 
end
...
for iSj=1:nSj
  ...
  if dotaskglm==1
    if exist('trialdef', 'var') && ~isempty(trialdef)
      v=zeros(length(vpath{iSj}), K);
      for ik=1:K
        v(:,ik) = vpath{iSj}==ik;
      end

      if iscell(trialdef.twindow) % we have multiple ERFs to account for
          for i_event = 1:length(trialdef.twindow)
              erf = compute_erf(v', trialdef.events{i_event, iSj}, trialdef.twindow{i_event}, trialdef.baseline{i_event});
              v = transpose(subtract_erf(v', erf,  trialdef.events{i_event, iSj}, trialdef.twindow{i_event}));
          end
      else
          erf = compute_erf(v', trialdef.events{iSj}, trialdef.twindow, trialdef.baseline);
          v = transpose(subtract_erf(v', erf,  trialdef.events{iSj}, trialdef.twindow));
      end
        v_demean = v;
