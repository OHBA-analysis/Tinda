...
elseif whichstudy==5
  % load second level HMM from HCP resting state
  temp2 = load('/ohba/pi/mwoolrich/datasets/HCP_CH_2022/ve_output_rest/secondLevelHMM_Poiss_window125_K3_overlappingWindows.mat');
  if strcmp(task, 'Wrkmem')
    % get file list TODO: Why are there 204 files, where there's only 79
    % subjects??
    load('/ohba/pi/mwoolrich/mvanes/Projects/Tinda/Study5/filelist_Wrkmem.mat');
    vpathfilename='/ohba/pi/mwoolrich/mvanes/Projects/Tinda/Study5/vpath_Wrkmem.mat';
  elseif strcmp(task, 'StoryM')
    load('/ohba/pi/mwoolrich/mvanes/Projects/Tinda/Study5/filelist_StoryM.mat');
    vpathfilename='/ohba/pi/mwoolrich/mvanes/Projects/Tinda/Study5/vpath_StoryM.mat';
  end
  config.nSj = length(mat_files_orth);
  if isfile(vpathfilename)
    load(vpathfilename)
  end
end
clear temp vpath;
...
for subnum=1:config.nSj
  ...
  elseif whichstudy==5
    if strcmp(task,'Wrkmem')
      if ~isfile(vpathfilename)
        temp = load(mat_files_orth{subnum},'T', 'X');
        hmmT{subnum} = temp.T - (length(hmm.train.embeddedlags)-1);
        vpath{subnum} = hmmdecode(temp.X,temp.T,hmm,1);
      end
    elseif strcmp(task, 'StoryM')
...
if whichstudy>5 && ~isfile(vpathfilename)
  save(vpathfilename, 'vpath', 'hmmT')
end
...
if whichstudy==52
  for subnum=1:config.nSj  
    ...
    T = T_poiss(1)*ones(length(hmmT{subnum}),1);
    save(mat_files_poiss{iSj},'X','T');
  end
end
...
if whichstudy~=4
  ...
  hmmPoiss = options.hmm;% rmfield(options.hmm,'Gamma');
  save([config.figdir,'secondLevelHMM_stoch_Poiss_window',num2str(W),'_K',int2str(options.K),overlapstring,'.mat'],'hmmPoiss');
