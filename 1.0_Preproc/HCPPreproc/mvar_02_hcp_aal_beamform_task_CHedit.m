...
% Updated by Cameron Higgins, Mark Hymers and Andrew Quinn
...
function mvar_02_hcp_aal_beamform_task_CHedit(subID)
...
for itask=2:3
    if itask==1
        megDir       = fullfile(conf.DIR_HCP, subID,  '/MEG/Motort/tmegpreproc/')
        conf.DIR_RAWVEDATA = '/ohba/pi/mwoolrich/datasets/HCP_CH/ve_output_Motort/'
        icaResDir = fullfile(conf.DIR_HCP, subID, '/MEG/Motort/icaclass/');
    elseif itask==2
        megDir       = fullfile(conf.DIR_HCP, subID,  '/MEG/StoryM/tmegpreproc/')
        conf.DIR_RAWVEDATA = '/ohba/pi/mwoolrich/datasets/HCP_CH/ve_output_StoryM/'
        icaResDir = fullfile(conf.DIR_HCP, subID, '/MEG/StoryM/icaclass/');
    else
        megDir       = fullfile(conf.DIR_HCP, subID,  '/MEG/Wrkmem/tmegpreproc/')
        conf.DIR_RAWVEDATA = '/ohba/pi/mwoolrich/datasets/HCP_CH/ve_output_Wrkmem/'
        icaResDir = fullfile(conf.DIR_HCP, subID, '/MEG/Wrkmem/icaclass/');
    end
    anatomyDir   = fullfile(conf.DIR_HCP, subID,  '/MEG/anatomy/');
    rNoiseDir    = fullfile(conf.DIR_HCP, subID, '/MEG/Noise/2_Pnoise/4D/');
    sessionList  = osl_filelist(megDir, '*.mat');


    megFileList    = [sessionList];
    
    to_remove = contains(megFileList,'trialinfo');
    megFileList = megFileList(~to_remove);
    if itask==1
        to_keep = contains(megFileList,'10-') | contains(megFileList,'11-');
    elseif itask==2
        to_keep = contains(megFileList,'8-') | contains(megFileList,'9-');
    else
        to_keep = contains(megFileList,'6-') | contains(megFileList,'7-');
    end
    megFileList = megFileList(to_keep)
    ...
    %     end
        ...
        filteredData = cell(size(data.trial));
        trialtime = cell(size(data.time));
        trialtime2 = cell(size(data.time));
        T = zeros(length(data.trial),1);
        T_downsampled = zeros(length(data.trial),1);
        % find longest trial:
        for itr=1:length(data.trial)
            T(itr) = size(data.trial{itr},2);
        end
        Tmax = max(T);
        allfiltereddat = [];
        alldat = [];
        for itr=1:length(data.trial)
            nanmask = any(isnan(data.trial{itr}),1);
            allData_filt = data.trial{itr}(:,~nanmask);
            % use wide-band reconstruction
            N   = 5;
            temp  = ft_preproc_bandpassfilter(allData_filt, data.fsample, recon_Fbp, N);
         %   filteredNoise = ft_preproc_bandpassfilter(dsNoise, Fs, recon_Fbp, N);
            filteredData{itr} = zeros(size(allData_filt,1),Tmax);
            nanmask2 = true(1,Tmax);
            nanmask2(1:T(itr)) = nanmask;
            filteredData{itr}(:,~nanmask2) = temp;
            allfiltereddat = [allfiltereddat,temp];
            trialtime{itr} = data.time{itr}(1) + [0:Tmax-1]/data.fsample;
            
            % downsample here:
            if ~isempty(temp)
                downsampledata = resample(temp',reducedFs,round(data.fsample));
            else
                downsampledata = [];
            end
            T_downsampled(itr) = length(downsampledata);
            alldat = [alldat;downsampledata];
            trialtime2{itr} = data.time{itr}(1) + [0:T_downsampled(itr)-1]/reducedFs;
        end
        ...
        sourcedata           = data;
        %sourcedata.trial     = {cat(2, filteredData{:})};
        sourcedata.trial     = filteredData;
        sourcedata.trial     = {alldat'};
        %trialdata = zeros(size(allData));
        %trialdata(:,~nanmask) = filteredData;
        sourcedata.trialinfo = [nan 0];
        sourcedata.time      = {cat(2,trialtime{:})}; % not real time, due to bad segments having been removed
        sourcedata.fsample   = reducedFs;
        ...
        if isfile(fullfile(icaResDir, icaClassFile))
    ...
    %     nii.resample_flirt(niiFileName, niiFileName_2mm, 2, 'trilinear', mni_brain_2mm);
        ...
        if ~exist(conf.DIR_RAWVEDATA)
            mkdir(conf.DIR_RAWVEDATA);
        end
        outfname = strcat(fullfile(conf.DIR_RAWVEDATA, prefix(fname, 'roinet')), '.mathdf5');
        save(outfname, 'data', 'sample_rate','T_downsampled','trialtime2', '-v7.3');
