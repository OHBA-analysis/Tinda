...
addpath(genpath('/Users/matsvanes/Documents/Werk/scripts/Higgins2020_Neuron'));
...
%
if exist(fullfile(config.hmmfolder,sprintf('vpath%d.mat', whichstudy)))
  load(fullfile(config.hmmfolder,sprintf('vpath%d.mat', whichstudy)))
else
...
twindow=20; % time window to do the fft on.
cfg=[];
cfg.method = 'mtmfft';
cfg.output = 'pow';
cfg.taper = 'dpss';
cfg.tapsmofrq = 0.1;
cfg.foi = 0:1/twindow:5;


for iSj=1:config.nSj
  % replace the state count in the viterbi path by its position on the
  % unit circle (according to the best sequence)
  vpcircle = vpath{iSj};
  for k=1:12
  ...
  psd{iSj}=[];
  dat=[];
  dat.time=1/fs:1/fs:twindow;
  dat.label{1} = 'circle_position';
  dat.fsample=fs;
  ...
  for i=1:length(hmmT{iSj})
    ...
    if T_poiss(end)>1
      if Noverlap==0
        X{cnt} = reshape(vpcircle(on_t:(W*T_poiss(end)+on_t-1),:),[W,T_poiss(end)]);
      else
        Xcircle = zeros(W,T_poiss(end));
    ...
    % do the frequency analysis stuff on X
    overlap = 1; %sec
    l = 10*fs;
    for il = 1:overlap*fs:length(X{cnt})-twindow*fs
      x = X{cnt}(il:il+twindow*fs-1);
      trial = [trial; x];
    end
    cnt=cnt+1;
  end
  if ~isempty(trial)
    ...
    dat.trial(:,1,:) = nan(1,length(dat.time)); % if there are no time windows long enough, trial will be empty
  end
  psd{iSj} = ft_freqanalysis(cfg, dat);
end
...
if whichstudy==4
  ...
  save(mat_files_poiss{iSj},'Xcircle');
end
clear X
